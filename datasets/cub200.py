...
def Give(opt, datapath):
    ...
    if opt.backdoor: #for poison train
        train_dataset = BaseDataset(train_image_dict, opt, is_train_poison=True)
    ...
    test_poison_dataset = None
    train_poison_dataset = None
    train_clean_dataset = None
    test_clean_dataset  = None
    if opt.backdoor: #for inference

        train_clean_dataset = BaseDataset(train_image_dict, opt)

        train_poison_dataset = BaseDataset(train_image_dict,  opt, is_test_poison=True) #

        test_clean_dataset = BaseDataset(test_image_dict, opt)

        test_poison_dataset = BaseDataset(test_image_dict,  opt, is_test_poison=True) #
    ...
    return {'training':train_dataset, 'validation':val_dataset, 'testing':test_dataset, 'evaluation':eval_dataset, 'evaluation_train':eval_train_dataset,
            'training_clean':train_clean_dataset, 'training_poison':train_poison_dataset, 'testing_clean':test_clean_dataset, 'testing_poison':test_poison_dataset}

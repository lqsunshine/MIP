...
if opt.savename=='group_plus_seed':
    if opt.log_online:
        ...
        opt.savename = opt.group+'_s{}'.format(opt.seed) #change
...
dataloaders['testing_poison']    = torch.utils.data.DataLoader(datasets['testing_poison'],    num_workers=opt.kernels, batch_size=opt.bs, shuffle=False)
...
# print(datasets['training'].image_list[0])
...
label_center_dict = None
if opt.loss == 'poison_triplet_c':
    label_center_dict = cluster.get_label_center_dict(opt, model,dataloaders['training'].dataset.avail_classes,dataloaders['training'].dataset.image_list,
                                  dataloaders['training'].dataset.get_normal_input)
...
for epoch in range(opt.n_epochs):
    ...
    for i,out in enumerate(data_iterator):
        ...
        #
        loss_args['indices'] = input_indices
        loss_args['get_trigger_input'] = dataloaders['training'].dataset.get_trigger_input
        loss_args['get_normal_input'] = dataloaders['training'].dataset.get_normal_input
        loss_args['label_center_dict']             = label_center_dict
        loss_args['model']             = model
        #
        loss_args['image_list'] = dataloaders['training'].dataset.image_list
        loss_args['image_dict'] = dataloaders['training'].dataset.image_dict
        #
    ...
    eval.evaluate(opt.dataset, LOG, metric_computer, [dataloaders['testing']],    model, opt, opt.evaltypes, opt.device, log_key='Test', Note="{0} Epoch (Test):".format(epoch))
    if opt.backdoor:
        print('\nComputing Testing Attack Metrics...')
        eval.evaluate(opt.dataset, LOG, metric_computer, [dataloaders['testing'],dataloaders['testing_poison']], model, opt, opt.evaltypes,
                      opt.device, log_key='Test',Note="{0} Epoch (Test:C->P):".format(epoch))
